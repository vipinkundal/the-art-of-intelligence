...
${items.map((item) => `            <li>${escapeHtml(item)}</li>`).join("\n")}
  ...
  function renderTopic(target) {
    const topicId = target.dataset.generativeTopic;
    const topics = window.generativeModellingTopics || [];
    const topic = topics.find((item) => item.id === topicId);

    if (!topic) {
      target.outerHTML = `<main class="lesson-main">
        <article class="lesson-article">
          <p class="eyebrow">Generative Modelling</p>
          <h1>Topic not found</h1>
          <p class="lesson-summary">The requested Generative Modelling topic could not be found.</p>
          <nav class="lesson-nav" aria-label="Lesson navigation">
            <a href="../../index.html">Generative Modelling</a>
    ...
    template.dataset.eyebrow = topic.groupLabel;
...
`;
  ...
  document.querySelectorAll("[data-generative-topic]").forEach(renderTopic);
})();
